...
public class MergeMeshes : MonoBehaviour
{
	...
	public void Merge (bool immediate)
	{
		...
		foreach (MeshFilter filter in filters)
		{
			...
			if (enableTint && mc != null)
			{
				// Randomize the b and a channels of the colors to add tint it supported.
				float randVal1 = Random.value;
				float randVal2 = Random.value;

				for (int i = 0, imax = mc.Length; i < imax; ++i)
				{
					Color vertexCol = mc[i];
					vertexCol.b = randVal1;
					vertexCol.a = randVal2;
					mc[i] = vertexCol;
				}
			}
		...
		if (verts.Length > 0)
		{
			...
			// If we're re-merging, remove the suffix
			int bracket = mName.IndexOf('(');
			if (bracket != -1) mName = mName.Substring(0, bracket - 1);
